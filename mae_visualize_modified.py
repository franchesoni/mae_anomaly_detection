...
def prepare_to_save(image):
    ...
    return np.array(torch.clip((image * imagenet_std + imagenet_mean) * 255, 0, 255)).astype(np.uint8)
...
def run_one_image(img, mask, model):
    ...
    # run MAE
    loss, y, mask = model(x.float(), mask_ratio=mask)  # we use mask_ratio to pass the mask
    ...
    return original, masked, reconstruction, reconstructionplusvisible
